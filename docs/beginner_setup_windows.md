...
テンプレートExcelが複数ある場合、自動で選べないことがあります。テンプレートExcelは `materials/input/template/` に1つだけ置き、`materials/input/` 直下には置かないでください。

損益計算書CSVや貸借対照表CSVが複数ある場合は、CSV内の月列で期間を判別します。該当3か月とその前の3か月のように月列が違うファイルは両方読み込み、同じ期間のファイルが複数ある場合だけ、ファイル名の日付・時刻が新しいものを使います。支給控除一覧表CSVは対象月のファイルを複数読み込みます。
